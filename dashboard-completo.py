...
def carregar_dados_csv(arquivo):
    # 1ª tentativa: latin-1 ;  |  2ª: detecção automática
    try:
    ...
    # Datas (aceita múltiplos formatos)
    ...
    # Normaliza categorias usadas nos gráficos (evita tipos mistos e NaN)
    df["CD_EQUIPTO"] = (
...
if g1.empty:
    ...
    g1["CD_CLASMANU"] = g1["CD_CLASMANU"].astype(str)
    st.altair_chart(
        alt.Chart(g1).mark_bar().encode(
            y=alt.Y("CD_CLASMANU:N", sort="-x", title="CD_CLASMANU"),
...
if g2.empty:
    ...
    g2["CD_EQUIPTO"] = g2["CD_EQUIPTO"].astype(str)
    st.altair_chart(
...
if g3.empty:
    ...
    g3["CD_EQUIPTO"] = g3["CD_EQUIPTO"].astype(str)
    st.altair_chart(
...
g6 = df.groupby("Ano/Mes").size().reset_index(name="Quantidade") if "Ano/Mes" in df.columns else pd.DataFrame()
if g6.empty:
