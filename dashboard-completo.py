...
def carregar_dados(arquivo):
    ...
    # Chaves categóricas usadas nos gráficos
    df["CD_EQUIPTO"] = (
...
PLANNED_REGEX = re.compile(
    r"(preventiv|primar|inspec|lubrif|preditiv|planejad|programad|plano\s*de\s*manut|"
    r"\bpm\d*\b|\bpm\b|\brevis[aã]o)",
    flags=re.IGNORECASE
)
...
def aplicar_filtro_nao_programadas(df: pd.DataFrame):
    ...
    if cols:
        ...
        # fallback: usa descrição para detectar "plano/revisão/preventiva"
...
def build_rules():
    rules_patterns = {
        # Novas categorias que capturam grande parte das Não Classificadas
        "Estrutural/Chassi": [
            ...
            r"\bhaste(s)?\b",
            r"\bestirante(s)?\b",
            r"\btrinca(d|s|)\b|\bquebr(ad|ou|a|ado)\b",
            r"\bchassi\b",
        ],
        ...
        "Cabine/Carroceria": [
            r"\bparabris|vidro|retrovisor|escada|porta(s)?\b", r"\bcap[oô]\b", r"\bgrade\b"
        ],
        "Tanque/Combustível (sem vazamento)": [
            r"\btanque\b", r"\bcinta do tanque\b", r"\bboia do tanque\b"
        ],
        # Categorias já existentes
        "Vazamento - Óleo": [
            r"\bvaz[a-z]*\b.*\boleo\b", r"\boleo\b.*\bvaz[a-z]*\b",
            r"\bretentor\b", r"\bvedador(es)?\b"
        ],
        "Vazamento - Hidráulico": [
            r"\bvaz[a-z]*\b.*\bhidraul", r"\bhidraul.*\bvaz[a-z]*\b",
            r"\bcilindro hidraul", r"\bbomba hidraul"
        ],
        "Vazamento - Combustível": [
            r"\bvaz[a-z]*\b.*\b(diesel|combust|gasol)",
            r"\b(diesel|combust|gasol).*\bvaz[a-z]*\b"
        ],
        "Freio": [
            r"\bfreio(s)?\b", r"\bpastilh", r"\blona(s)?\b",
            r"\bdisco(s)?\b", r"\btambor(es)?\b", r"\bpin[cç]a\b",
            r"\bcilindro mestre\b", r"\bfluido de freio\b"
        ],
        "Suspensão": [
            r"\bamortecedor(es)?\b", r"\bmola(s)?\b", r"\bfeixe de mola\b",
            r"\bbucha(s)?\b", r"\bbandeja\b", r"\bpivo\b", r"\bestabilizador\b"
        ],
        "Direção": [
            r"\b(caixa|sistema) de dire[cç][aã]o\b",
            r"\bterminal de dire[cç][aã]o\b",
            r"\bbarra de dire[cç][aã]o\b", r"\borbitrol\b"
        ],
        "Elétrica": [
            r"\beletr[aí]c", r"\bchicote\b", r"\bfus[ií]vel\b", r"\brele\b",
            r"\bl[aâ]mpada|farol|lanterna\b", r"\bbateria\b",
            r"\bmotor de arranque\b", r"\balternador\b"
        ],
        "Falha Eletrônica / Painel": [
            r"\bpainel\b", r"\b(luz|lamp)\s*espia\b", r"\bc[oó]digo de falha\b",
            r"\b(sensor|atuador|modulo|ecu|can)\b", r"\bsem comunica[cç][aã]o\b",
            r"\binjet(or|or(es)?)\b"
        ],
        "Sistema Hidráulico (sem vazamento)": [
            r"\bbomba\b.*\bhidraul", r"\bvalvula\b.*\bhidraul",
            r"\bcilindro\b.*\bhidraul", r"\bhidromotor|hidrostat(ico|ica)\b"
        ],
        "Pneus/Rodagem": [
            r"\bpneu(s)?\b", r"\broda(s)?\b", r"\bc[aâ]mara\b",
            r"\bcalibr", r"\bfuro\b"
        ],
        "Rodantes": [
            r"\brodante(s)?\b", r"\brolete(s)?\b", r"\broda motriz\b",
            r"\bcoroa\b", r"\besteira|sapata\b"
        ],
        "Ar Condicionado": [
            r"\bar condicionado\b|\bac\b", r"\bcompressor\b.*\bar\b|\bcompressor do ar\b",
            r"\bcondensador\b", r"\bevaporador\b", r"\bventilador\b", r"\bgas do ar\b"
        ],
        "Transmissão / Câmbio": [
            r"\b(cambio|transmiss[aã]o)\b", r"\bembreagem\b",
            r"\b(diferencial|planet[aá]ria|coroa|pinhao)\b", r"\bcarda?n\b"
        ],
        "Motor": [
            r"\bmotor(?!ista)\b", r"\bcabecote\b", r"\bpist[aã]o\b", r"\bbiela\b",
            r"\bbronzina\b", r"\bbomba de oleo\b", r"\barrefe(c|ç)edor\b",
            r"\bturbina|turbo\b", r"\bcorreia dent"
        ],
...
def ml_reclass_optional(df_base: pd.DataFrame, col_txt: str, col_cat_in: str, threshold: float = 0.6):
    ...
    try:
        ...
        # sklearn não disponível — retorna a coluna de entrada sem alterações
...
mask_periodo = (
    (df["ENTRADA"] >= pd.to_datetime(data_inicio)) &
    (df["ENTRADA"] <= pd.to_datetime(data_fim))
